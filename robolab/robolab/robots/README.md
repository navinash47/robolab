...
4. **Sensors** — URDF does not express MuJoCo rangefinders. Lidar is implemented via `mujoco.mj_ray` (MuJoCo) and `pybullet.rayTest` (PyBullet) with the same 5 planar angles.
5. **Drive model (Phase 1 / Phase 5):** Both adapters use **kinematic planar drive** (pose update) rather than torque/velocity wheel actuators. Wheel joints are visual. This keeps training stable for the lidar wall-follow gate; full contact dynamics can be revisited later without changing the URDF or TaskSpec.
